...
For Modern Greek, Dilemma's MG lookup tables sometimes use a different
canonical lemma than Greek Wiktionary (e.g., `τρώγω` vs `τρώω` for
"eat", `λέγω` vs `λέω` for "say"). Downstream consumers like
[Lemma](https://github.com/ciscoriordan/lemma) handle this mismatch
with a separate MG equivalences file generated by cross-referencing
dilemma's `mg_lookup_scored.json` against Wiktionary headwords, using
corpus frequency as a tiebreaker for the canonical form.
