...
For Modern Greek, dilemma's MG lookup tables can disagree with Greek
Wiktionary on the canonical lemma because English Wiktionary is
processed first in `build_data.py` and its lemma choice wins under
the "first wins" merge strategy. EN Wiktionary editors tend to use
the fuller morphological stem (`τρώγω`, `λέγω`) while EL Wiktionary
uses the modern contracted form (`τρώω`, `λέω`). This affects ~6,300
lemmas. Downstream consumers like
[Lemma](https://github.com/ciscoriordan/lemma) handle this with a
separate MG equivalences file generated by cross-referencing
`mg_lookup_scored.json` against Wiktionary headwords, using corpus
